  ...
  return (
    <AppCard className="xl:row-span-2">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="grid size-10 place-items-center rounded-2xl bg-cyan-50 text-cyan-600">
...
function ScoreMeter({ checked }: { checked: boolean }) {
  return (
    <div className="relative mx-auto h-56 max-w-sm animate-[meterFade_0.45s_ease-out]">
      <div className="absolute inset-x-8 top-5 h-32 rounded-t-full border-[1.15rem] border-b-0 border-slate-200" />
      <div className="absolute inset-x-8 top-5 h-32 rounded-t-full border-[1.15rem] border-b-0 border-transparent border-l-emerald-400 border-t-cyan-500" />
      <div className="absolute left-1/2 top-[7.55rem] size-5 -translate-x-1/2 rounded-full bg-cyan-500 shadow-lg shadow-cyan-200" />
      <div
        className="absolute left-1/2 top-[7.82rem] h-3 w-28 origin-left rounded-full bg-cyan-500 transition-transform duration-1000 ease-out [clip-path:polygon(0_50%,100%_0,100%_100%)]"
      ...
      <div className="absolute bottom-5 left-1/2 -translate-x-1/2 text-center">
        {checked ? (
          <>
            <p className="text-3xl font-black text-slate-950">782</p>
